...
class WP_Query {
	...
	public function get_posts() {
		...
		if ( $q['posts_per_page'] < -1 ) {
			$q['posts_per_page'] = abs( $q['posts_per_page'] ); } elseif ( $q['posts_per_page'] == 0 ) {
			$q['posts_per_page'] = 1; }
			...
			if ( $this->is_tax ) {
				if ( empty( $post_type ) ) {
					...
					if ( ! $post_type ) {
						$post_type = 'any'; } elseif ( count( $post_type ) == 1 ) {
	...
	public function is_author( $author = '' ) {
		...
		if ( in_array( (string) $author_obj->ID, $author ) ) {
			return true; } elseif ( in_array( $author_obj->nickname, $author ) ) {
			return true; } elseif ( in_array( $author_obj->user_nicename, $author ) ) {
				return true; }
	...
	public function is_category( $category = '' ) {
		...
		if ( in_array( (string) $cat_obj->term_id, $category ) ) {
			return true; } elseif ( in_array( $cat_obj->name, $category ) ) {
			return true; } elseif ( in_array( $cat_obj->slug, $category ) ) {
				return true; }
	...
	public function is_tag( $tag = '' ) {
		...
		if ( in_array( (string) $tag_obj->term_id, $tag ) ) {
			return true; } elseif ( in_array( $tag_obj->name, $tag ) ) {
			return true; } elseif ( in_array( $tag_obj->slug, $tag ) ) {
				return true; }
	...
	public function is_front_page() {
		...
		if ( 'posts' == get_option( 'show_on_front' ) && $this->is_home() ) {
			return true; } elseif ( 'page' == get_option( 'show_on_front' ) && get_option( 'page_on_front' ) && $this->is_page( get_option( 'page_on_front' ) ) ) {
			return true;
			} else { 			return false; }
	}
