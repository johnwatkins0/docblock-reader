	...
	foreach ( (array) $post_links_temp as $link_test ) {
		if ( ! in_array( $link_test, $pung ) ) { // If we haven't pung it already
			...
			if ( false === $test ) {
				continue; }
			if ( isset( $test['query'] ) ) {
				$post_links[] = $link_test; } elseif ( isset( $test['path'] ) && ( $test['path'] != '/' ) &&  ($test['path'] != '' ) ) {
...
function _http_build_query( $data, $prefix = null, $sep = null, $key = '', $urlencode = true ) {
	...
	foreach ( (array) $data as $k => $v ) {
		...
		if ( ! empty( $key ) ) {
			$k = $key . '%5B' . $k . '%5D'; }
		if ( $v === null ) {
			continue; } elseif ( $v === false ) {
			$v = '0'; }

			if ( is_array( $v ) || is_object( $v ) ) {
				array_push( $ret,_http_build_query( $v, '', $sep, $k, $urlencode ) ); } elseif ( $urlencode ) {
...
function _default_wp_die_handler( $message, $title = '', $args = array() ) {
	...
	if ( ! did_action( 'admin_head' ) ) :
		...
		if ( isset( $r['text_direction'] ) && 'rtl' == $r['text_direction'] ) {
			$text_direction = 'rtl'; } elseif ( function_exists( 'is_rtl' ) && is_rtl() ) {
			$text_direction = 'rtl'; }
?>
<!DOCTYPE html>
...
function wp_parse_args( $args, $defaults = '' ) {
	if ( is_object( $args ) ) {
		$r = get_object_vars( $args ); } elseif ( is_array( $args ) ) {
		$r =& $args;
